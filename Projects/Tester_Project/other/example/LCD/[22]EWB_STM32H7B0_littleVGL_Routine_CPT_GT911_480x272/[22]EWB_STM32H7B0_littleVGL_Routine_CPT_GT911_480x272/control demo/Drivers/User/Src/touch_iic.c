...
void Touch_IIC_GPIO_Config (void)
{
	...
	Touch_IIC_SCL_CLK_ENABLE; //Initialize IO port clock
	...
	GPIO_InitStruct.Pull = GPIO_NOPULL; // without pull-up and pull-down
	GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW; // speed level
...
void Touch_IIC_ACK(void)
{
	...
	Touch_IIC_SCL(0); // When the SCL output is low, SDA should be pulled high immediately to release the bus
	Touch_IIC_SDA(1);		
...
uint8_t Touch_IIC_WaitACK(void)
{
	...
	if( HAL_GPIO_ReadPin(Touch_IIC_SDA_PORT,Touch_IIC_SDA_PIN) != 0) //Determine whether the device responds		
	{
		Touch_IIC_SCL(0);	
		Touch_IIC_Delay( IIC_DelayVaule );		
	...
	else
	{
		Touch_IIC_SCL(0);	
		Touch_IIC_Delay( IIC_DelayVaule );		
		return ACK_OK; //The response is normal
	}
}
...
* Return value: ACK_OK - the device responded OK
...
* Note: 1. The high bit comes first
...
uint8_t Touch_IIC_ReadByte(uint8_t ACK_Mode)
{
	...
	if ( ACK_Mode == 1 ) // acknowledgment signal
		Touch_IIC_ACK();
	else
