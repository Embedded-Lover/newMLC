...
>>>>> Function description: 
	...
	* 2. Run the official routine lv_demo_benchmark for benchmark performance testing 
...
void MPU_Config(void); // MPU configuration 
...
{ 
	...
	KEY_Init(); // Initialize the key pin 
...
// LCD_PWMinit(40); // Backlight pin PWM initialization, duty cycle 40%		  
	...
	lv_demo_benchmark(); // Run the official routine lv_demo_benchmark for benchmark performance testing
	...
	{ 
		...
		LED1_Toggle; // LED indication 
		HAL_Delay(20); // GT911 touch screen scan interval should not be less than 10ms, it is recommended to set it to 20ms 
	} 
...
{ 
  ...
  */ 
  ...
  /* Set the LTDC clock, here is set to 33MHz, that is, the refresh rate is around 60 frames, too high or too low will cause flicker*/ 
