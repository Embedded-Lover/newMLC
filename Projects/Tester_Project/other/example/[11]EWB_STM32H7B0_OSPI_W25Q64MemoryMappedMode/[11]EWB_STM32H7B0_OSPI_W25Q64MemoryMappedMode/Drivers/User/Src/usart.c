...
void HAL_UART_MspInit(UART_HandleTypeDef* huart)
{
	...
	{
		__HAL_RCC_USART1_CLK_ENABLE();		// Turn on USART1 clock

		GPIO_USART1_TX_CLK_ENABLE;				// Turn on GPIO clock for USART1 TX pin
		GPIO_USART1_RX_CLK_ENABLE;				// Turn on GPIO clock for USART1 RX pin

		GPIO_InitStruct.Pin 			= USART1_TX_PIN;					// TX pin
		GPIO_InitStruct.Mode 		= GPIO_MODE_AF_PP;				// Multiplexed push-pull output
		GPIO_InitStruct.Pull 		= GPIO_PULLUP;						// pull up
		GPIO_InitStruct.Speed 		= GPIO_SPEED_FREQ_VERY_HIGH;	// speed class 
...
* Function: Redirect the fputc function to use the printf function
* Description: None
**************************************************************************************************/
